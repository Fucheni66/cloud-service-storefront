// 登录页认证逻辑：账号密码登录 / 邮箱验证码登录 / 邮箱注册 / 找回密码 / Google 登录。
(function () {
  ...
  function bindEmailAuth() {
    bindSubmit('login-form', handleEmailLogin);
    bindSubmit('login-code-form', handleEmailCodeLogin);
    bindSubmit('register-form', handleEmailRegister);
    bindSubmit('forgot-form', handlePasswordReset);

    // 所有「获取验证码」按钮：按 data-scene + data-email-input 通用处理。
    document.querySelectorAll('[data-send-code]').forEach(function (button) {
      button.addEventListener('click', function () {
        handleSendCode(button);
      });
    });
  }
  ...
  async function handleEmailCodeLogin(event) {
    ...
    try {
      const data = await postAuthJson(config.email.codeLoginPath, {
        email: getInputValue('login-code-email'),
        code: getInputValue('login-code-input'),
      });
      saveLoginInfo(data);
      showAuthMessage('登录成功，正在进入控制台。', 'success');
      redirectAfterLogin();
    } catch (error) {
      showAuthMessage(error.message || '验证码登录失败，请重试。', 'error');
    } finally {
      setButtonLoading(submitButton, false, '登录');
    }
  }
  ...
  async function handlePasswordReset(event) {
    ...
    showAuthMessage('正在重置密码...', 'info');

    try {
      await postAuthJson(config.email.resetPath, {
        email: getInputValue('forgot-email'),
        code: getInputValue('forgot-code'),
        password: getInputValue('forgot-password'),
      });
      showAuthMessage('密码已重置，请使用新密码登录。', 'success');
      ...
      showAuthMessage(error.message || '密码重置失败，请重试。', 'error');
    } finally {
      setButtonLoading(submitButton, false, '重置密码');
    }
  }

  async function handleSendCode(button) {
    const emailInputId = button.dataset.emailInput;
    const scene = button.dataset.scene || 'register';
    const email = getInputValue(emailInputId);
    ...
    try {
      await postAuthJson(config.email.codePath, { email, scene });
      showAuthMessage('验证码已发送至邮箱，请查收（10 分钟内有效）。', 'success');
  ...
  function redirectAfterLogin() {
    window.setTimeout(function () {
      window.location.href =
        getLoginRedirectUrl() || config.loginSuccessPage || 'console.jsp';
    }, 500);
  }
