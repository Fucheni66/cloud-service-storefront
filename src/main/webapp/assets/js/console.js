...
function redirectHashToManagePage() {
  ...
  window.location.replace(`console-manage.jsp?id=${encodeURIComponent(serviceId)}`);
  return true;
}
...
function renderEmptyState() {
  return $('<div>', { class: 'lg:col-span-2 bg-white rounded-xl border border-gray-200 shadow-sm p-10 text-center' }).append(
    ...
    $('<a>', { href: 'products.jsp', class: 'inline-flex items-center justify-center mt-5 bg-primary hover:bg-primary-hover text-white px-5 py-2.5 rounded text-sm font-medium transition shadow-sm', text: '去购买服务' })
  )[0];
}
  ...
  return $('<article>', { class: 'bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition overflow-hidden' }).append(
    ...
    $('<div>', { class: 'px-6 py-4 bg-gray-50 flex flex-col sm:flex-row sm:items-center justify-between gap-3' }).append(
      ...
      $('<div>', { class: 'flex gap-2' }).append(
        $('<a>', { href: 'purchase.jsp', class: 'px-3 py-2 rounded border border-gray-300 bg-white text-gray-700 hover:text-primary hover:border-primary text-sm transition', text: '续费' }),
        $('<a>', { href: `console-manage.jsp?id=${encodeURIComponent(service.id)}`, class: 'px-3 py-2 rounded bg-primary text-white hover:bg-primary-hover text-sm transition', text: '管理' })
      )
    )
  )[0];
}
